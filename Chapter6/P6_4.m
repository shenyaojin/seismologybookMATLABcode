...
v11=4;v21=4;H2=2;  %速度为4km的高速夹度，层厚为2km
b2=(v21-v11)/H2;    %该层的速度梯度
v12=v20;v22=5.5;H3=10;  %速度从3km/s到.5km/s的速度变化层，层厚为12km
b3=(v22-v12)/H3; %该层的速度梯度
x=[0 60];     %将震中距分为2个结点
v=[];
for z=0:16
if(z<=H1)
    v=[v;ones(1,2)*v10+b1*z];  %根据震中距的2个结点构建第一层速度
elseif(z>H1&z<=(H2+H1))
    v=[v;ones(1,2)*(v11+b2*(z-H1))]; %根据震中距的2个结点构建第二层速度
elseif(z>(H2+H1)&z<=(H1+H2+H3))
    v=[v;ones(1,2)*(v12+b3*(z-(H1+H2)))]; %根据震中距的2个结点构建第三层速度
end
end
pcolor(x,[0:16],v);   %绘制速度结构
